...
let nuevaNoticia = unaNoticia => {
    return `
    <article class="${(unaNoticia.tipoNacional)?"nacional":"internacional"}">
        <img class="imagen" src="${unaNoticia.imgUrl}" alt="">
        <h2>${unaNoticia.titulo}</h2>
        <p class="fecha-noticia">${unaNoticia.fecha}</p>
        <p>${unaNoticia.descripcion}</p>
    </article>
    `
}

let contenedorNacional = document.querySelector(".contenedorNacional");
let contenedorInternacional = document.querySelector(".contenedorInternacional");

for (let noticia of noticias) {
    switch(noticia.tipoNacional){
        case false:
            contenedorInternacional.innerHTML += nuevaNoticia(noticia);
        break;
        case true:
            contenedorNacional.innerHTML += nuevaNoticia(noticia);
        break;
    }
}